...
# Get data configuration data_config_path=config/coco.data文件存储的是：类别数量、训练集图片路径脚本、验证集图片路径脚本、
#类别名字脚本等信息，parse_data_config函数在程序parse_config.py里，data_config是一个字典，存储了这些值
data_config = parse_data_config(opt.data_config_path)
# 得到测试数据集脚本路径 data/coco/5k.txt
...
# Get dataloader,ListDataset(test_path)是一个自定义的继承自torch.utils.data.Dataset的类，定义了数据的接口
