...
def non_max_suppression(prediction, num_classes, conf_thres=0.5, nms_thres=0.4):
    """
    函数的输入为预测结果、num_classes（这里是 80）、置信度（objectness 分数阈值）和 nms_thres（NMS IoU 阈值）。
    ...
    #From (center x, center y, w, h) to (x1, y1, x2, y2)用每个框的两个对角坐标能更轻松地计算两个框的 IoU
    ...
    for image_i, image_pred in enumerate(prediction):
        # 过滤低于置信度（objectness 分数阈值）的预测框，即保留>= conf_thres的image_pred
        conf_mask = (image_pred[:, 4] >= conf_thres).squeeze()
        image_pred = image_pred[conf_mask]
        # If none are remaining => process next image（进入下一个循环，即下一个图片了）
        if not image_pred.size(0):
            continue
        # 找到留下来的每个预测值的最大得分对应的类别及类别得分
        class_conf, class_pred = torch.max(image_pred[:, 5 : 5 + num_classes], 1, keepdim=True)
        #  把每个已有最大得分的类的预测值按照格式(x1, y1, x2, y2, obj_conf, class_conf, class_pred)cat成detections
        detections = torch.cat((image_pred[:, :5], class_conf.float(), class_pred.float()), 1)
        # 预测值的类别标号用unique()除去重复的元素，即一类只留下一个元素
        ...
        for c in unique_labels:
            ...
            # Sort the detections by maximum objectness confidence对某个类，将obj_conf从大到小排序
...
nGT: 真实标签方框个数
nCorrect：正确检测出来的个数
mask:mask用于后面来标记真正负责预测出目标的anchor
conf_mask:当这个mask负责一个目标检测时，将conf_mask对应元素设置为1
tx,ty: 真实方框与物体中心所在格子左上角的偏移量
tw,th：真实方框与anchor的宽高缩放比值的对数值
tconf：当一个anchor真正负责检测一个目标时，令tconf中对应这个anchor位置的元素为1，表示这里有一个目标
tcls：用于记录每个anchor的对应目标的类别
"""
...
def build_targets(
    ...
    # 生成一个维度为2x3x13x13的元素为1的tensor，conf_mask用于标记含有目标得分的tensor中对应的所有anchors中，一个mask是否真正
    # 负责一个目标检测。当这个mask负责一个目标检测时，将conf_mask对应元素设置为1
    # conf_mask后面要先对与真实方框IOU大于0.5的anchor的conf_mask位置初始为0，然后又将IOU最大的那个位置标记为1
    conf_mask = torch.ones(nB, nA, nG, nG)
    tx = torch.zeros(nB, nA, nG, nG)
    ty = torch.zeros(nB, nA, nG, nG)
    tw = torch.zeros(nB, nA, nG, nG)
    th = torch.zeros(nB, nA, nG, nG)
    # 生成一个维度为2x3x13x13的元素为0的tensor，当一个anchor真正负责检测一个目标时，令tconf中对应这个anchor位置的元素为1，表示这里有一个目标
    ...
    for b in range(nB):
        ...
        for t in range(target.shape[1]):
            ...
            # 当标签是真的时候，nGT加1
            nGT += 1
            #将真实标签中的x,y,w,h转换为相对于输入yolo层特征图的坐标，这里nG=13。
            #gx,gy是相对于yolo层特征图的真实方框的中心坐标。gw,gh为真实标签方框相对于输入yolo层特征图的大小
            gx = target[b, t, 1] * nG
            gy = target[b, t, 2] * nG
            gw = target[b, t, 3] * nG
            gh = target[b, t, 4] * nG
            # Get grid box indices，对gx,gy向下取整，得到标签中心点在13x13个格子中所在格子的左上角坐标
            gi = int(gx)
            gj = int(gy)
            # Get shape of gt box，得到标签方框的宽高tensor，变成一个二维tensor，维度为1x4，前两个元素为0，后两个元素为方框宽高
            ...
            # Coordinates
            # tx[b, best_n, gj, gi]，ty[b, best_n, gj, gi]表示真正负责检测这个标签方框目标的anchor所对应的真实标签
            # 与这个标签所在格子左上角在x,y方向上的偏移量
